...
```{r,echo=FALSE, warning=FALSE, fig.cap ="Trend Line Graph for Counties", fig.dim=c(5,7) }
library(ggplot2)
library(ggrepel)
time1= read.csv("casesperpop.csv")
ggplot(time1, mapping = aes(x = Combined_Key, y = value))+geom_line(aes(color = variable), size = 0.8) +
  theme(legend.position = "none")+xlim(0, 120)+geom_text_repel(data = subset(time1, Combined_Key == max(Combined_Key)),aes(label= variable),nudge_x = 30,segment.color = NA)+labs(
...
```{r, echo=FALSE, warning=FALSE, fig.cap="Density Graph for Number of Cases"}
